...
public class MemorySegment implements Comparable<MemorySegment> {
	...
	public void get(byte[] buffer, long addr, int length) {
		checkBounds(addr);
		dataBuffer.position(index(addr));
		dataBuffer.get(buffer, 0, length);
	}
	
	public void get(ByteBuffer buffer, long addr) {
		checkBounds(addr);
		buffer.rewind();
		buffer.put(this.data, index(addr), buffer.capacity());
	}
	...
	public static class UnaddressableLocationException extends RuntimeException {
